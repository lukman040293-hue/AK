...
export default function EmployeeApp() {
  ...
  const handlePointChange = (index, field, value) => {
    ...
    setUForm(p => {
      const newPoints = [...p.points];
      newPoints[index] = { ...newPoints[index], [field]: sanitized };
      return { ...p, points: newPoints };
    });
  };
  ...
  const handleUnifiedSubmit = async (e) => {
    ...
    setIsProcessing(true);
    try {
      const segmenName = uForm.namaSegmen.trim() || 'Segmen 1';
      const startLat = parseFloat(uForm.points[0].lat) || null;
      const startLng = parseFloat(uForm.points[0].lng) || null;
      const endLat = parseFloat(uForm.points[1].lat) || null;
      const endLng = parseFloat(uForm.points[1].lng) || null;

      // A. UPDATE TABEL PROJECTS (SINKRONISASI PETA)
      const { data: projData, error: fetchErr } = await supabase.from('projects').select('actual_segments_data').eq('id', closestProject.id).single();
      if (fetchErr) throw fetchErr;

      let actualSegments = projData.actual_segments_data;
      if (typeof actualSegments === 'string') {
         try { actualSegments = JSON.parse(actualSegments); } catch(err) { actualSegments = []; }
      }
      if (!Array.isArray(actualSegments)) actualSegments = [];

      const existingIndex = actualSegments.findIndex(s => (s.name || s.id) === segmenName);
      const newSegmentData = {
          id: Date.now(),
          name: segmenName,
          points: (startLat && startLng) ? [{ lat: startLat, lng: startLng }] : [],
          boundary_end: (endLat && endLng) ? { lat: endLat, lng: endLng } : null
      };

      if (existingIndex >= 0) {
          actualSegments[existingIndex] = { ...actualSegments[existingIndex], ...newSegmentData };
      } else {
          actualSegments.push(newSegmentData);
      }
      ...
      const projectUpdatePayload = {
          ...
          end_lat: endLat,
          end_lng: endLng,
          actual_segments_data: actualSegments,
          updated_at: new Date().toISOString()
      };

      const { error: projUpdateErr } = await supabase.from('projects').update(projectUpdatePayload).eq('id', closestProject.id);
      if (projUpdateErr) throw projUpdateErr;

      // B. INSERT KE TABEL FIELD_REPORTS (LOG & MEDIA)
      // 1. Log Utama Wajib
      const descUtama = `Tim lapangan telah mengirimkan data hasil survei untuk ${segmenName}.\nPanjang Eks.: ${uForm.panjang || '-'}m | Lebar Eks.: ${uForm.lebar || '-'}m | Model Eks.: ${uForm.jenis_model_awal || '-'}\nKoordinat: Awal(${startLat || '-'}, ${startLng || '-'}) ... Akhir(${endLat || '-'}, ${endLng || '-'})`;
      
      await supabase.from('field_reports').insert([{
          project_id: closestProject.id,
          title: 'Pengiriman Data Survei',
          description: descUtama,
          is_problem: false
      }]);

      // 2. Log Catatan
      if (uForm.noteDesc.trim()) {
          await supabase.from('field_reports').insert([{
              project_id: closestProject.id,
      ...
      // 3. Log Dokumentasi (Media)
      if (uMedia.length > 0) {
        showMsg(`Mengunggah ${uMedia.length} dokumentasi...`, 'info');
        for (let i = 0; i < uMedia.length; i++) {
          const file = uMedia[i];
          const compressed = await compressImage(file);
          const fileName = `survei_${Date.now()}_${Math.random().toString(36).substring(7)}.${compressed.name.split('.').pop()}`;
          
          await supabase.storage.from('project-media').upload(`reports/${fileName}`, compressed);
          const { data } = supabase.storage.from('project-media').getPublicUrl(`reports/${fileName}`);
          
          await supabase.from('field_reports').insert([{
              project_id: closestProject.id,
              title: `Dokumentasi Survei 0% (Part ${i + 1})`,
              description: `Foto dokumentasi survei segmen ${segmenName}`,
      ...
      // C. INSERT KE TABEL DOCUMENTS (FILE CSV)
      if (uDataUkur) {
        showMsg('Mengunggah data ukur CSV...', 'info');
        ...
        await supabase.from('documents').insert([{
            project_id: closestProject.id,
            name: uDataUkur.name || 'Data Ukur Survei',
            category: 'Survei',
            status: 'Verified',
      ...
      showMsg('Data Survei berhasil disinkronkan ke Dashboard!', 'success');
                ...
                <form onSubmit={handleUnifiedSubmit} className="space-y-4 text-left w-full">
                  ...
                  <SurveyInputRow label="Titik Awal">
                    <div className="flex gap-2 w-full">
                      <input type="text" placeholder="Lat" value={uForm.points[0].lat} onChange={e => handlePointChange(0, 'lat', e.target.value)} className="flex-1 min-w-0 p-2.5 text-xs rounded-xl border border-slate-200 bg-slate-50 outline-none focus:border-blue-400 focus:bg-white" />
                      <input type="text" placeholder="Lng" value={uForm.points[0].lng} onChange={e => handlePointChange(0, 'lng', e.target.value)} className="flex-1 min-w-0 p-2.5 text-xs rounded-xl border border-slate-200 bg-slate-50 outline-none focus:border-blue-400 focus:bg-white" />
                  ...
                  <SurveyInputRow label="Titik Akhir">
                    <div className="flex gap-2 w-full">
                      <input type="text" placeholder="Lat" value={uForm.points[1].lat} onChange={e => handlePointChange(1, 'lat', e.target.value)} className="flex-1 min-w-0 p-2.5 text-xs rounded-xl border border-slate-200 bg-slate-50 outline-none focus:border-blue-400 focus:bg-white" />
                      <input type="text" placeholder="Lng" value={uForm.points[1].lng} onChange={e => handlePointChange(1, 'lng', e.target.value)} className="flex-1 min-w-0 p-2.5 text-xs rounded-xl border border-slate-200 bg-slate-50 outline-none focus:border-blue-400 focus:bg-white" />
